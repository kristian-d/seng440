...
ycc_image_t *rgb_to_ycc(uint8_t *img, int width, int height) {
  ...
  register int y_index = 0, c_index = 0;
  register int physical_width = width*3;
  register int img_bytes = height*physical_width;
  register int limit, pixel_index;
  register uint16x8_t y_acc;
  register int16x8_t c_acc;
  register uint8x8_t y_final, c_final;
  uint8x8x3_t intlv_rgb;
  for (pixel_index = 0; pixel_index < img_bytes;) {
