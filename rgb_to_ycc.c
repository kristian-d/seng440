...
ycc_image_t *rgb_to_ycc(uint8_t *img, int width, int height) {
  ...
  for (int pixel_index = 0; pixel_index < img_bytes;) {
    for(limit = pixel_index + physical_width; pixel_index < limit; pixel_index += 24) { // Loops over one row.
      ...
      c_acc = vmlaq_u16(c_acc, vreinterpretq_s16_u16(vmull_u8(intlv_rgb.val[0], c_rcoeff)), c_rcoeffsign);
      c_acc = vmlaq_u16(c_acc, vreinterpretq_s16_u16(vmull_u8(intlv_rgb.val[1], c_gcoeff)), c_gcoeffsign);
      c_acc = vmlaq_u16(c_acc, vreinterpretq_s16_u16(vmull_u8(intlv_rgb.val[2], c_bcoeff)), c_bcoeffsign);
