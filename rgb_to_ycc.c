...
void rgb_image_compare(uint8_t *rgb_image_1, uint8_t *rgb_image_2, int width, int height, int print) {
  ...
  for (int i = 0; i < height; i++) { // TODO change this back to use height, not 1
    ...
    for (int j = 0; j < width*3; j += 3) {
      ...
      if (print || flag) printf("(%d->%d,%d->%d,%d->%d) ", red1, red2, green1, green2, blue1, blue2);
      if (flag && !print) {
...
ycc_image_t *rgb_to_ycc(uint8_t *img, int width, int height) {
  // for testing to see if image is being read in properly
  // rgb_image_print_hex(img, width, height);
  ...
  for (int row_start_index = 0; row_start_index < img_bytes; row_start_index += physical_width) { // RGB -> YCC
    ...
    for (int pixel_index = row_start_index; pixel_index < row_start_index + physical_width; pixel_index += 3) {
      ...
      //ignore: for debugging y[y_index++] = 16 + (((2155872*r + 4227858*g + 822084*b) + (1 << 22)) >> 23); // y can be written to consecutively, so ++ is ok
      //ignore: for debugging y[y_index++] = 16 + ((16763*r + 32909*g + 6391*b) >> 16); // y can be written to consecutively, so ++ is ok
      ...
      //ignore: for debugging cb[cb_index++] = 128 + (((-1241514*r + -2441085*g + 3682599*b) + (1 << 22)) >> 23); // cb can be written to consecutively, so ++ is ok
      //ignore: for debugging cb[cb_index++] = 128 + ((-9676*r + -18996*g + 28672*b) >> 16); // cb can be written to consecutively, so ++ is ok
      ...
      //ignore: for debugging cr[cr_index++] = 128 + (((3682599*r + -3087008*g + -595591*b) + (1 << 22)) >> 23); // cr can be written to consecutively, so ++ is ok
      //ignore: for debugging cr[cr_index++] = 128 + ((28672*r + -24009*g + -4662*b) >> 16); // cr can be written to consecutively, so ++ is ok
      ...
      //ignore: for debugging y[y_index++] = 16 + (((2155872*img[pixel_index] + 4227858*img[pixel_index + 1] + 822084*img[pixel_index + 2]) + (1 << 22)) >> 23); // y can be written to consecutively, so ++ is ok
      //ignore: for debugging y[y_index++] = 16 + ((16763*img[pixel_index] + 32909*img[pixel_index + 1] + 6391*img[pixel_index + 2]) >> 16); // y can be written to consecutively, so ++ is ok
    }
...
void ycc_image_debug(ycc_image_t *ycc_image) {
  ...
  printf("YCC image stats: pixel_count=%d, width=%d, height=%d, y_bytes=%d, cb_bytes=%d, cr_bytes=%d, total_bytes=%d\n", ycc_image->height*ycc_image->width, ycc_image->width, ycc_image->height, ycc_image->y_bytes, ycc_image->cb_bytes, ycc_image->cr_bytes, ycc_image->total_bytes);
...
uint8_t add8_saturating(uint8_t a, uint8_t b) {
  uint8_t c = a + b;
  if (c < a) c = -1;
  return c;
}

uint8_t sub8_saturating(uint8_t a, uint8_t b) {
  uint8_t c = a - b;
  if (c > a) c = 0;
  return c;
}
...
uint8_t *ycc_to_rgb(ycc_image_t *ycc_image) {
  ...
  for (int i = 0; i < ycc_image->height; i++) {
    for (int j = 0; j < ycc_image->width; j++) {
      ...
      cb = ycc_image->cb[(int)i/2 * ycc_image->width/2 + (int)j/2];
      cr = ycc_image->cr[(int)i/2 * ycc_image->width/2 + (int)j/2];
      rgb_image[rgb_pixel_index] = add8_saturating(1.164*(y - 16), 1.596*(cr - 128));
      rgb_image[rgb_pixel_index + 1] = sub8_saturating(sub8_saturating(1.164*(y - 16), 0.813*(cr - 128)), 0.391*(cb - 128));
      rgb_image[rgb_pixel_index + 2] = add8_saturating(1.164*(y - 16), 2.018*(cb - 128));
    }
  }
  return rgb_image;
}
